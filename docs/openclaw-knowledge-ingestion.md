...
本文档描述 OpenClaw 如何把自身沉淀的知识接入 `myLocalRAG`。

目标不是让 OpenClaw 直接改写 `vault/`，而是让 OpenClaw 输出更接近 `myLocalRAG` 的知识采集协议，再由 `myLocalRAG` 完成去重、审核、升格和发布。
...
但这些内容进入 `myLocalRAG` 时，应先作为 Raw Inbox 条目，而不是直接成为正式 wiki 页面。

### 1.2 myLocalRAG 负责知识生命周期
...
更推荐写入一个可导入的 OpenClaw 知识目录，或直接调用 `myLocalRAG` 的 Raw Inbox API。
...
`intakeStage` 决定进入 `myLocalRAG` 后的默认去向。
...
OpenClaw 的 Markdown 正文建议保持稳定结构，方便 `myLocalRAG` 后续抽取 evidence atom。
...
`myLocalRAG` Raw Inbox 当前可通过如下 API 写入：
...
### Phase 2: myLocalRAG 导入器

`myLocalRAG` 提供 OpenClaw importer：
...
如果 OpenClaw 希望知道哪些内容已经被 `myLocalRAG` 升格，可以维护一个状态文件：
...
6. `.obsidian/` 不参与导入。
7. 保持文件路径稳定，方便 `myLocalRAG` 生成稳定 id。
...
```text
OpenClaw: 生产结构化知识原料
myLocalRAG: 接收入 Raw Inbox，去重、审核、升格、发布
Obsidian: 阅读和同步发布层
```

OpenClaw 不需要理解 `myLocalRAG` 的完整 wiki 生成逻辑，只要输出带元数据的 Markdown 或 manifest。

`myLocalRAG` 也不应该把 OpenClaw 内容视为已经审核过的正式 wiki，而是把它作为高质量外部采集源接入现有知识工作台。
